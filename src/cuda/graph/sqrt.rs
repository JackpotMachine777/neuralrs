...
const KERNEL: &str = r#"
    extern "C" __global__ void vsqrt(float* out, const float* x, const size_t n) {
        size_t i = blockIdx.x * blockDim.x + threadIdx.x;
        if(i < n) out[i] = sqrtf(x[i]);
    }

    extern "C" __global__ void sqrt_grad(float* out, const float* g, const float* x, const size_t n) {
        size_t i = blockIdx.x * blockDim.x + threadIdx.x;
        if(i < n) out[i] = g[i] / (2.0f * sqrtf(x[i]));
    }
"#;
...
pub fn sqrt(a: &Rc<RefCell<Node>>) -> Rc<RefCell<Node>> {
    ...
    let (out_data, shape, len) = {
        let a_n = a.borrow();
        let a_gpu = a_n.gpu.as_ref().expect("cuda sqrt: input not on GPU");
        let len: usize = a_n.shape.iter().product();
        let mut out = stream.alloc_zeros::<f32>(len).expect("cuda sqrt: alloc failed");
        let f = module().load_function("vsqrt").expect("vsqrt not found");
        let cfg = LaunchConfig::for_num_elems(len as u32);
        let mut bd = stream.launch_builder(&f);
        bd.arg(&mut out); bd.arg(&a_gpu.data); bd.arg(&len);
        unsafe { bd.launch(cfg).expect("cuda sqrt: launch failed"); }
    ...
    let out_node = Node::new(vec![], shape);
    {
        ...
        let grad = Rc::new(RefCell::new(stream.alloc_zeros::<f32>(len).expect("cuda sqrt: grad alloc")));
        ...
        n.backward_fn = Some(Box::new(move |_grad: &Vec<f32>| {
            ...
            let local = {
                ...
                let x = &a_n.gpu.as_ref().expect("cuda sqrt bwd: input not on GPU").data;
                let mut out = stream.alloc_zeros::<f32>(len).expect("cuda sqrt bwd: alloc");
                let f = module().load_function("sqrt_grad").expect("sqrt_grad not found");
                let cfg = LaunchConfig::for_num_elems(len as u32);
                let mut bd = stream.launch_builder(&f);
                bd.arg(&mut out); bd.arg(&*g); bd.arg(x); bd.arg(&len);
                unsafe { bd.launch(cfg).expect("cuda sqrt bwd: launch"); }
                out
            };
