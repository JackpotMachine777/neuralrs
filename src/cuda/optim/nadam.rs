//! NAdam over resident parameters (mirror of CPU `NAdam`).
...
const KERNEL: &str = r#"
    extern "C" __global__ void nadam(float* param, const float* grad, float* m, float* v,
        ...
        if(i < n) {
            ...
            float m_hat = m[i] / bc1;
            float v_hat = v[i] / bc2;
            float m_nesterov = beta1 * m_hat + (1.0f - beta1) * g / bc1;
            param[i] = param[i] - lr * m_nesterov / (sqrtf(v_hat) + eps);
        }
    }
"#;
crate::kernel_module!(KERNEL);

/// NAdam — Adam with Nesterov-style momentum. In-place update.
pub struct NAdam {
    pub lr: f32,
    pub beta1: f32,
    pub beta2: f32,
    ...
    pub fn step(&mut self, params: &[Rc<RefCell<Node>>]) {
        ...
        if self.t == 0 {
            for p in params {
                ...
                self.m.push(stream.alloc_zeros::<f32>(len).expect("nadam: m alloc failed"));
                self.v.push(stream.alloc_zeros::<f32>(len).expect("nadam: v alloc failed"));
            }
        }
        assert_eq!(params.len(), self.m.len(), "nadam: parameter count changed between steps");
        ...
        let f = module().load_function("nadam").expect("nadam not found");

        for (i, p) in params.iter().enumerate() {
            let mut node = p.borrow_mut();
            let len: usize = node.shape.iter().product();
            let gpu = node.gpu.as_mut().expect("nadam: param not on GPU");
            ...
            builder.arg(&bc2);
            builder.arg(&n);
            unsafe { builder.launch(cfg).expect("nadam: launch failed"); }
        }
        self.t += 1;
    }
}