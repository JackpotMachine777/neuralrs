...
///
/// Backward uses the softmax Jacobian trick: for each row,
/// `grad_in = out * (grad - sum(out * grad))`, which is the clean closed form
/// instead of building the full Jacobian matrix.
